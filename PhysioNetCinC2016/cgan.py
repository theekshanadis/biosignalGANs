...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        y = int(self.indexes[index].split('_')[1])

        return x.squeeze(dim=0), y
...
class Discriminator(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv1d(2, 12, kernel_size=3, bias=True)
    ...
    def forward(self, x, label):
        label = label.unsqueeze(dim=1).repeat(1, 1, x.shape[-1])
        x = x.unsqueeze(dim=1)
...
class Generator(torch.nn.Module):
    def __init__(self, z):
        ...
        self.linear2 = torch.nn.Linear(z + 1, 200)

    def forward(self, x, cls_emb, id1, id2, id3, id4, id5):
        x = self.linear2(torch.cat([x, cls_emb], dim=1))
...
epoch = 120
try:
    generator.load_state_dict(torch.load('./Models/generator-{:d}.tm'.format(epoch)))
    torch.save(generator.state_dict(), './FModels/cgan_gen-{}.tm'.format(code))
...
for epoch in range(400):
    ...
    for i, (x, labels) in enumerate(training_generator):    
        ...
        gen_labels = torch.rand((x.shape[0], 1)).cuda()
        gen_labels = torch.where(gen_labels > 0.5, 1.0, 0.0)
        ...
        x_recons = generator(z.cuda(), gen_labels, id1.cuda(), id2.cuda(), id3.cuda(), id4.cuda(), id5.cuda())
        g_loss = adverserial_loss(discriminator(x_recons, gen_labels), valid)
        ...
        real_loss = adverserial_loss(discriminator(x.cuda(), labels.float().cuda()), valid)
        fake_loss = adverserial_loss(discriminator(x_recons.detach(), gen_labels), fake)
        ...
        if i == 1:
            ...
            for k in range(50):
                ...
                plt.savefig('./SIN/T2/V3/f_{:d}.png'.format(k))
                plt.close()
                plt.clf()
    ...
    torch.save(generator.state_dict(), './Models/generator-{:d}.tm'.format(epoch))
    torch.save(discriminator.state_dict(), './Models/discriminator-{:d}.tm'.format(epoch))
    torch.save(optimizer_D.state_dict(), './Models/opt_d-{:d}.tm'.format(epoch))
    torch.save(optimizer_G.state_dict(), './Models/opt_g-{:d}.tm'.format(epoch))
